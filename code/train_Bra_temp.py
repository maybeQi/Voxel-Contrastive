...
parser.add_argument('--exp', type=str,
                    default='Bra_CPCL_general_staLoss',
...
class PrototypeContrastiveLearning(nn.Module):
    ...
    def forward(self, query, positive_key, negative_keys):
        if negative_keys is not None:
            ...
            negative_keys = (negative_keys.view(2,4,256))  # (4, 2, 256) -> (2, 256, 2)
            negative_keys_transposed = negative_keys.transpose(-1,-2)

            # 矩阵乘法计算相似度
            negative_logits = torch.matmul(query, negative_keys_transposed)  # (2, 4, 5)
            ...
            loss = 0.5*F.cross_entropy(logits / self.temperature, labels)
        else:
            loss = torch.zeros(())
        return loss
...
def train(args, snapshot_path):
    ...
    # pcl_model = PrototypeContrastiveLearning(temperature=0.1)
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # print('unlabel data feature map shape:', unlabel_fts.shape)
            all_fts_confidence = []
            ...
            # for class_index in range(num_classes):
            #     label_fts_proto = getPrototype(label_fts, (unlabel_preMask == class_index),
            #                                    all_fts_confidence[class_index]).detach()
            #     label_fts_prototypes.append(label_fts_proto)

            #my label qury
            label_query = []
            for class_index in range(num_classes):
                query_prototype = getPrototype(unlabel_fts_st, (label_batch[args.labeled_bs:] == class_index),
            ...
            positive_keys = []
            for class_index in range(num_classes):
                positive_prototype = getPrototype(unlabel_fts, (unlabel_preMask == class_index),
            ...
            neg_fts_prototypes = torch.stack(neg_fts_prototypes, dim=1)
            # print(neg_fts_prototypes.shape)
            if i_batch != 0:
            ...
            loss = supervised_loss + consistency_weight *0.4*ct_loss
            ...
            # 打印日志
            logging.info(
                'iteration %d : loss : %f, loss_ce: %f, loss_dice: %f, ct_loss: %f' %
                (iter_num, loss.item(), loss_ce.item(), loss_dice.item(),ct_loss.item()))
