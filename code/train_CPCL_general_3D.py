...
from loss_contrast_mem import PixelContrastLoss
...
parser.add_argument('--root_path', type=str,
                    default='../data/LA_data_h5', help='Name of Experiment--数据集路径')
parser.add_argument('--exp', type=str,
                    default='LA_CPCL_general_staLoss_moon',
                    help='experiment_name--自定义stable_loss')  # _staLoss: --stability_loss 1
parser.add_argument('--model', type=str,
                    default='vnet', help='model_name')
parser.add_argument('--max_iterations', type=int,
                    default=10000, help='maximum epoch number to train--起初为10000')
...
parser.add_argument('--seed', type=int, default=1120, help='random seed')
...
parser.add_argument('--consistency_rampup', type=float,
                    default=40.0, help='consistency_rampup--控制一致性的上升,默认40')
...
def get_current_consistency_weight(epoch):
    ...
    return args.consistency * ramps.sigmoid_rampup(epoch, args.consistency_rampup)  # 后面参数表示宽度


def get_current_pixelloss_weight(epoch):
    return args.consistency * ramps.sigmoid_rampdown(epoch, args.consistency_rampup)  # 后面参数表示宽度
...
def getPrototype(features, mask, class_confidence):
    ...
    masked_features = torch.mul(fts,
                                mask_new)  # here use a broadcast mechanism: https://blog.csdn.net/da_kao_la/article/details/87484403
...
def calDist(fts_adj_size, prototype):
    ...
    prototype_new = prototype_new.unsqueeze(4)    #[C,1]变为[C,1,1,1,1]
...
# 计算源域原型与目标特征之间的预测
def prototype_pred(src_prototypes, feature_tgt, mask_src, class_nums):  # src表示来源
    #  extract the foreground features via masked average pooling
    ...
    for class_index in range(class_nums):
        dist = calDist(feature_tgt, src_prototypes[class_index]).unsqueeze(1)  # 每个样本对应一个类别的相似度
        final_dist = dist if class_index == 0 else torch.cat((final_dist, dist), 1)  # 拼接，其中包含了每个样本对每个类别的相似度
    final_dist_soft = torch.softmax(final_dist, dim=1)  # 前面没有归一化
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            label_Mask = torch.argmax(outputs_soft[:2], dim=1)
            s_unlabel_Mask = torch.argmax(outputs_soft[2:], dim=1)
            ...
            # noise = torch.clamp(torch.randn_like(            ##加上一个服从均值为0、标准差为0.1的正态分布随机数
            ...
            # stride = volume_batch_r.shape[0] // 2          ##整数除法向下取整
            # preds = torch.zeros([stride * T, 2, d, w, h]).cuda()            # stride为batchsize大小，2*最好理解为2×，两列
            # for i in range(T//2):
            #     ema_inputs = volume_batch_r   + \
            #         torch.clamp(torch.randn_like(volume_batch_r) * 0.1, -0.2, 0.2)
            #     with torch.no_grad():
            #         preds[2 * stride * i:2 * stride *
            #               (i + 1)] = model(ema_inputs)                 #这里的2为repeat的2
            ...
            ########get prototype###########
            # outfeats = model.featuremap_d9
            # outfeats = model.out_feature[:args.labeled_bs]
            # print(outfeats.shape)
            # _, c_f, _, _, _ = outfeats.shape
            # prototype_bank = torch.zeros(batch_size, num_classes, c_f).cuda()
            # index = torch.argmax(preds,dim=1)
            # # print("predslabel",predslabel)
            # for ba in range(batch_size):
            #     for n_class in range(num_classes):
            #         mask_temp = (index[ba] == n_class).float()
            #         top_fea = outfeats[ba] * mask_temp
            #         prototype_bank[ba, n_class] = top_fea.sum(-1).sum(-1).sum(-1) / (mask_temp.sum() + 1e-6)
            # print(prototype_bank.shape)

            ############################
            # labeled-to-unlabeled process
            label_fts = model.featuremap_center[:args.labeled_bs]
            label_pixel_fts = model.featuremap_d9

            #特征融合
            unlabel_fts = ema_model.featuremap_center.to(device)
            unlabel_pixel_fts = ema_model.featuremap_d9.to(device)
            middle_feature_upsampled = F.interpolate(
                unlabel_fts, size=(112, 112, 80), mode='trilinear', align_corners=True )  # 上采样到 (112, 112, 80, 256)
            conv1x1x1 = torch.nn.Conv3d(16, 256, kernel_size=1).to(device)
            extended_feature_256 = conv1x1x1(unlabel_pixel_fts)
            sim_fts = middle_feature_upsampled + extended_feature_256
            ...
            #  labeled-to-unlabeled prediction
            ...
            unlabel_preMask = torch.argmax(ema_output_soft, dim=1)  # ema mask
            ...
            # prototype-based unlabeled-to-labeled prediction  该剔除的
            unlabeltolabel_pred = prototype_pred(unlabel_fts_prototypes, sim_fts, unlabel_preMask, num_classes)
            unlabeltolabel_pred = torch.argmax(unlabeltolabel_pred,dim=1)
            pixelloss = PixelContrastLoss()
            unsup_pixel_loss = pixelloss(sim_fts, unlabeltolabel_pred, s_unlabel_Mask)

            # U2L_loss = ce_loss(unlabeltolabel_pred, label_batch[:args.labeled_bs][:])

            # mask_t = torch.zeros_like(ema_output_soft).cuda()
            # for ba in range(2):
            #     for n_class in range(num_classes):
            #         class_prototype = unlabel_fts_prototypes[ba, n_class]    #之后把他改成bank
            #         mask_t[ba, n_class] = F.cosine_similarity(ema_output[ba],
            ...
            sup_pixel_loss = pixelloss(label_pixel_fts[:args.labeled_bs], label_batch[:args.labeled_bs][:], label_Mask)
            pixelloss_weight = get_current_pixelloss_weight(iter_num // 150)
            supervised_loss = 0.5 * (loss_dice + loss_ce)
            ...
            loss = supervised_loss + 0.6 * sup_pixel_loss + 0.6*consistency_weight*unsup_pixel_loss
            # loss = supervised_loss + consistency_weight * (L2U_consistency_loss + MT_stability_loss + consistency_loss) + protoalign_weight * U2L_loss
            # 梯度回传，更新梯度
            ...
            #                   U2L_loss, iter_num)
            writer.add_scalar('info/pixel_loss',
                              sup_pixel_loss, iter_num)
            writer.add_scalar('info/MT_stability_loss',
                              MT_stability_loss, iter_num)
            writer.add_scalar('info/consistency_weight',
                              consistency_weight, iter_num)
            writer.add_scalar('info/pixelloss_weight',
                              pixelloss_weight, iter_num)

            # 打印日志
            logging.info(
                'iteration %d : loss : %f, loss_ce: %f, loss_dice: %f, pixel_loss: %f ,unsup_pixel_loss: %f' %
                (iter_num, loss.item(), loss_ce.item(), loss_dice.item(), sup_pixel_loss.item(), unsup_pixel_loss.item()))
...
if __name__ == "__main__":
    ...
    snapshot_path = "../model/{}_{}.3/{}".format(
